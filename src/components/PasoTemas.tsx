...
type Tema = 'Inseguridad' | 'Economía' | 'Corrupción'| 'Social';
...
const TEMA_DESC: Record<Tema, string> = {
  ...
  Social: 'Aborto, nueva constitución, matrimonio igualitario',
};
...
export default function PasoTemas() {
  ...
  const [orden, setOrden] = useState<Tema[]>(['Inseguridad', 'Economía', 'Corrupción','Social' ]);
  ...
  const continuar = () => {
    ...
    if (igualPeso) {
      pesos = { Inseguridad: 1, Economía: 1, Corrupción: 1, Social: 1 };
    } else {
      ...
      pesos = {
        ...
        Social: total - orden.indexOf('Social'),
      };
    }
    setPesosTemas(pesos);
    irAPaso(2);
